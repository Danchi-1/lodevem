...
class HuggingFaceBackend(BenchmarkBackend):
    ...
    def load(cls, path: str | Path, allow_untrusted: bool = False) -> BenchmarkBackend:
        ...
        has_pickle = (path / "pytorch_model.bin").exists() or any(path.glob("*.bin")) or any(path.glob("*.pt"))

        if not has_safetensors and has_pickle and not allow_untrusted and not is_in_docker():
            raise SecurityPolicyError(
                f"Security Policy Violation: Hugging Face model repository '{path.name}' contains legacy pickled weights (.bin).\n"
        ...
        try:
            ...
            if is_causal_lm:
                try:
                    model = AutoModelForCausalLM.from_pretrained(str(path), device_map="cpu", **load_kwargs)
                except Exception:
                    model = AutoModelForCausalLM.from_pretrained(str(path), **load_kwargs)
                    model = model.to("cpu")
            else:
                try:
                    model = AutoModel.from_pretrained(str(path), device_map="cpu", **load_kwargs)
                except Exception:
                    model = AutoModel.from_pretrained(str(path), **load_kwargs)
                    model = model.to("cpu")
            ...
            tokenizer = None
            try:
                tokenizer = AutoTokenizer.from_pretrained(str(path), trust_remote_code=False)
            except Exception:
                if is_causal_lm:
    ...
    def generate_inputs(self, shape: tuple[int, ...] | None = None, **kwargs) -> Any:
        ...
        from lodevem.guards import assert_inputs_on_cpu
        assert_inputs_on_cpu(inputs)
        return inputs
